...
parser.add_argument(
    '--batch_size', default=128, type=common.positive_int,
    help='Batch size used during evaluation, adapt based on your memory usage.')

parser.add_argument(
    '--result_folder', default="/home/")
...
def main():
    ...
    query_pids, query_fids, que_views = common.load_dataset(args.query_dataset, None)
    gallery_pids, gallery_fids, gal_views = common.load_dataset(args.gallery_dataset, None)
...
#             pid_matches[mask] = False
            ...
            for i in range(len(distances)):
                ...
                # Save results to destination folder.
                with open(os.path.join(args.result_folder, fids[i].replace('.jpg','.txt')), "w") as fo:
                    for x in np.argsort(distances[i])[:100]:
                        fo.write("{:s},{:5f},{}\n".format(gallery_fids[x].replace('.jpg',''),distances[i][x],pid_matches[i,x]))
                    fo.close()
    # Save index.csv
    with open(os.path.join(args.result_folder,"index.csv"), "w") as fo:

...
def sample_k_fids_for_pid(pid, all_fids, all_pids, all_views, batch_k):
    ...
    selected_views= tf.gather(all_views, shuffled[:batch_k])
    return selected_fids, tf.fill([batch_k], pid), selected_views
...
def main():
    ...
    # We store all arguments in a json file. This has two advantages:s
    ...
    pids, fids, tracklet = common.load_dataset(args.train_set, args.image_root)
    
    ####  Load view_point information:
    file_name = "/home/hthieu/AICityChallenge2019/classifier/classifier_h5/track2_view_test.h5"
    f = h5py.File(file_name, 'r')
    scores = f['vehi_view']
    views = np.argmax(scores,axis=1)
    f.close()
    ...
    dataset = dataset.map(lambda pid: sample_k_fids_for_pid(
        pid, all_fids=fids, all_pids=pids, all_views = views, batch_k=args.batch_k))
    ...
    dataset = dataset.map(
        lambda fid, pid, view: common.fid_to_image_v2(
            fid, pid, view, image_root=args.image_root,
    ...
    if args.gramma_augment:
        dataset = dataset.map(
            lambda im, fid, pid, view: (data_augmentation.gramma_augmentation(im) , fid, pid,view))
            
    if args.crop_augment:
        dataset = dataset.map(
            lambda im, fid, pid, view: (tf.random_crop(im, net_input_size + (3,)), fid, pid,view))
    ...
    images, fids, pids, views = dataset.make_one_shot_iterator().get_next()
...
#         dists, pids, args.margin, batch_precision_at_k=args.batch_k-1)
    ...
    losses, train_top1, prec_at_k, _, neg_dists, pos_dists, db_info = loss.batch_hard_view_enhance(
        dists, pids, views, args.margin, batch_precision_at_k=args.batch_k-1)

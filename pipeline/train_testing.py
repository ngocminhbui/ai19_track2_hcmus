...
parser.add_argument(
    '--gramma_augment', action='store_true', default=False,
    help='When this flag is provided, gramma augmentation is performed.')
...
def main():
    ...
    pids, fids, views = common.load_dataset(args.train_set, args.image_root)
    ...
    print("Total instance ids: ",len(unique_pids))
    ...
    if args.gramma_augment:
        dataset = dataset.map(
            lambda im, fid, pid: (data_augmentation.gramma_augmentation(im) , fid, pid))
...
#         endpoints = head.head(endpoints, args.embedding_dim, is_training=True, npids = len(unique_pids))
...
#    batch_loss = tf.reduce_mean(losses)
    ...
    dists = loss.cdist(endpoints['emb'], endpoints['emb'], metric=args.metric)
    losses_batch_hard  = loss.LOSS_CHOICES['batch_hard'](
    dists, pids, args.margin, batch_precision_at_k=None)
    
    losses_batch_hard = tf.reduce_mean(losses_batch_hard)
    
    ##### TRIPLET CENTER LOSS ####
    losses_center = loss.LOSS_CHOICES[args.loss](
        endpoints['emb'], pids, args.margin, batch_precision_at_k=None)
    
    losses_center = tf.reduce_mean(losses_center)
    
    batch_loss = losses_center * 0.5 + losses_batch_hard
    
#     with tf.Session() as sess:
#         sess.run(tf.global_variables_initializer())
#         center, batch_hard = sess.run([losses_center, losses_batch_hard])
#     ipdb.set_trace()
    
    #     if ('softmax') in args.loss:
    #         print("!!!!! Warning softmax loss is using !!!!!")
    #         softmax_loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels = pids, logits = endpoints['logits'])
    #         losses = losses + softmax_loss * 0.1
...
#                             endpoints['emb'], losses, fids])

                _, summary, step, b_embs, b_loss_cen, b_loss_hard, b_fids = \
                    sess.run([train_op, merged_summary, global_step,
                            endpoints['emb'], losses_center, losses_batch_hard, fids])
                elapsed_time = time.time() - start_time
                
                b_loss = b_loss_cen + b_loss_hard
...
#     with tf.Session() as sess:
#         sess.run(tf.global_variables_initializer())
#         pids, emb, uni_pid, ins_mask, cen_emb, dists, positive_val, negative_val,diff = sess.run(loss.LOSS_CHOICES[args.loss](
